  ...
  return (
    <div className="statistik-summary-row">
      {data.map((item, index) => (
        <div
          key={index}
          className={`statistik-summary-card ${item.label.toLowerCase().replace(/\s+/g, '-')}`}
          style={{
        ...
        >
          <div className="statistik-summary-value" style={{ fontSize: '2.5rem', fontWeight: 700, marginBottom: '0.3rem' }}>
            {item.value}
          </div>
